...
import glob
import os
from utils import get_files_names
...
class AudioPreprocesser:
    def __init__(self, config_path: str, modes=['normalize', 'denoise', 'merge']):
    ...
    def normalize(self):
        for dataset_name in tqdm(self.config['datasets_names']):
            ...
            for raw_f in tqdm(raw_files):
                ...
                if not os.path.exists(dirname(normalize_path)):
                    os.mkdir(dirname(normalize_path))
                runcmd(
