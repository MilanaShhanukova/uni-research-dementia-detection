...
if __name__ == "__main__":
    ...
    with open(os.path.join(parsed_args.reports_folder, 'featured_model_text_golden.txt'), 'w+') as f:
    ...
    data_golden.to_excel(os.path.join(parsed_args.reports_folder, 'data_golden_predictions_featured_text_model.xlsx'))
    ...
    predictions = model.predict(data_test.drop(columns=test_drops))


    with open(os.path.join(parsed_args.reports_folder, 'featured_model_text_test.txt'), 'w+') as f:
        print(classification_report(data_test['condition'], predictions), file=f)
